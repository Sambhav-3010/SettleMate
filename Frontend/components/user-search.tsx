    ...
    <div className="space-y-3">
      <Popover open={open} onOpenChange={setOpen}>
        ...
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-0">
          <Command shouldFilter={false}>
            <CommandInput placeholder="Search by username or email..." value={query} onValueChange={setQuery} />
            <CommandList>
              <CommandEmpty>No users found.</CommandEmpty>
              <CommandGroup>
                {users.map((user) => (
                  <CommandItem key={user.id} value={user.username} onSelect={() => handleSelect(user)}>
      ...
      <div className="flex flex-wrap gap-2">
        {selectedUserObjects.map((user) => (
          <Badge key={user.id} variant="secondary" className="rounded-none border border-border px-2 py-1">
            {user.username}
            <Button variant="ghost" size="icon-sm" className="ml-1 h-4 w-4" onClick={() => handleRemove(user.id)}>
              <X className="h-3 w-3" />
            </Button>
          </Badge>
        ))}
